...
const ResultsList = ({ keyword, results = [], onReset, onDownload, hasReport }) => {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto flex flex-col h-[85vh]">
            {/* Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 p-4 bg-space-card/80 backdrop-blur-md rounded-2xl border border-white/10 sticky top-0 z-20 shadow-lg">
                <button
                    onClick={onReset}
                    className="flex items-center gap-2 text-blue-200 hover:text-white transition-colors group w-full sm:w-auto"
                >
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto">
                    {hasReport && (
                        <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            onClick={onDownload}
                            className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-accent-blue to-accent-purple text-white font-semibold rounded-xl hover:shadow-lg hover:shadow-accent-blue/30 transition-all"
                        >
                            <Download size={18} />
            ...
            <div className="flex flex-wrap gap-2 sm:gap-4 mb-4 px-2">
                <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-space-card/50 rounded-xl border border-white/10 flex-grow sm:flex-grow-0">
                    <FileText size={16} className="text-blue-300" />
                    <span className="text-white font-semibold">{results.length}</span>
                    <span className="text-blue-300 text-sm">Results</span>
                </div>
                <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-green-500/10 rounded-xl border border-green-500/20 flex-grow sm:flex-grow-0">
                    <CheckCircle size={16} className="text-green-400" />
                    <span className="text-green-400 font-semibold">{validCount}</span>
                    <span className="text-green-300/60 text-sm">Valid</span>
                </div>
                <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-red-500/10 rounded-xl border border-red-500/20 flex-grow sm:flex-grow-0">
                ...
                {results.length === 0 ? (
                    ...
                    results.map((item, index) => (
                        <motion.div
                            ...
                            className="p-4 sm:p-6 bg-gradient-to-r from-space-card/40 to-space-card/10 backdrop-blur-md rounded-3xl border border-white/5 hover:border-accent-blue/30 transition-all group hover:shadow-[0_0_20px_rgba(76,201,240,0.1)] relative overflow-hidden"
                        >
                            {/* Status indicator */}
